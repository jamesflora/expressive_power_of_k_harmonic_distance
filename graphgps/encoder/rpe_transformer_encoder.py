...
from torch_geometric.utils import to_dense_adj, to_dense_batch, to_networkx, get_laplacian, unbatch_edge_index
import logging
...
def get_batched_edge_indices(edge_index: torch.Tensor, batch: torch.Tensor) -> torch.Tensor:
    """
    Given:
        - edge_index: (2, num_edges) with global node indices
        - batch: (num_nodes,) assigning each node to a graph

    Returns:
        A (3, num_edges) tensor such that:
        - first row is the batch ID per edge
        - second and third rows are the per-graph (local) node indices of each edge
    """
    edge_batch = batch[edge_index[0]]  # graph ID of each edge
    num_nodes_per_graph = torch.bincount(batch)
    cum_nodes = torch.cumsum(num_nodes_per_graph, dim=0)
    cum_nodes = torch.cat([cum_nodes.new_zeros(1), cum_nodes[:-1]])
    relative_edge_index = edge_index - cum_nodes[edge_batch]
    return torch.cat([edge_batch[None, :], relative_edge_index], dim=0)
...
class LearnableKharmonicLayer(torch.nn.Module):
    def __init__(self, inf_distance : float = 512):
        super().__init__()
        self.inf_distance = inf_distance
        self.k = torch.nn.Parameter(torch.tensor([0.3]), requires_grad=True)
    ...
    def forward(
        ...
        """ Add the k-harmonic distance to the batch"""
        if not hasattr(data, "eigvecs") or not hasattr(data, "eigvals"):
            raise AttributeError(
                "Data has no attribute eigvals or eigvecs."
                " Data must first be preprocessed with the function add_laplacian_eigenpairs_to_data"
                " before Learnablekharmonic can be used."
            )
        #eigvecs_matrix = to_dense_adj(
        #    data.complete_graph_index,
        #    batch=data.batch,
        ...
        ##eigvals_diag = torch.diag_embed(eigvals ** -scaled_k)
        #eigvals_diag = torch.diag_embed(eigvals ** -1)
        #eigvecs_matrix_T = eigvecs_matrix.permute((0, 2, 1))
        #laplacian = torch.bmm(torch.bmm(eigvecs_matrix, eigvals_diag), eigvecs_matrix_T)
        ##logging.info(f"  {scaled_k}")
        #batch_edge_idx = get_batched_edge_indices(data.edge_index, data.batch) 
        ...
        #    batch_edge_idx[2] 
        #] 
        #data.test_edge_attr = edge_attr.unsqueeze(1)


        #test
        laplacian_edge_index, laplacian_edge_attr = get_laplacian(data.edge_index)
        laplacian_matrix = to_dense_adj(laplacian_edge_index, batch=data.batch, edge_attr=laplacian_edge_attr, max_num_nodes=None)
        pinv = torch.linalg.pinv(laplacian_matrix, hermitian=True)
        scaled_k = self.k * 5
        eigval, eigvec = torch.linalg.eigh(pinv)
        eigval = torch.clamp(eigval, min=0.0)
        eigval_frac = eigval ** scaled_k
        pinvsq = eigvec @ torch.diag_embed(eigval_frac) @ eigvec.transpose(-1,-2)

        pinv_diagonal = torch.diagonal(pinvsq, dim1=1, dim2=2)
        resistance_matrix = pinv_diagonal.unsqueeze(2) + pinv_diagonal.unsqueeze(1) - 2 * pinvsq
        #resistance_matrix = set_intercomponent_distance_to_inf(data, resistance_matrix, self.inf_distance)
        batched_idx = get_batched_edge_indices(data.edge_index, data.batch)
        eff_res = resistance_matrix[batched_idx[0], batched_idx[1], batched_idx[2]]
        data.extra_edge_attr = eff_res.unsqueeze(1)
        logging.info(f"  {scaled_k}")
...
@register_node_encoder("RPEMpnn")
class RPEMpnnEncoder(torch.nn.Sequential):
    def __init__(self, dim_emb, *args, **kwargs):
        encoders = []
        if cfg.posenc_RPE.learnable_kharmonic.enable:
            encoders.append(
                LearnableKharmonicLayer(
