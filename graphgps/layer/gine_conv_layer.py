...
from graphgps.loader.master_loader import edge_size
...
class GINEConvLayer(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dropout, residual):
        ...
        self.edge_encoder = nn.Linear(1, dim_in)
        self.edge_encoder2 = nn.Linear(4, dim_in)
        gin_nn = nn.Sequential(
            pyg_nn.Linear(dim_in, dim_out), nn.ReLU(),
            pyg_nn.Linear(dim_out, dim_out))
        self.model = pyg_nn.GINEConv(gin_nn, edge_dim=dim_in)

    def forward(self, batch):
        x_in = batch.x
        if batch.extra_edge_attr.shape[1] != self.dim_in:
            if cfg.dataset.format == 'PyG-ZINC':
                batch.edge_attr = self.edge_encoder(batch.extra_edge_attr)
            elif cfg.dataset.name == 'BREC':
                batch.edge_attr = self.edge_encoder(batch.extra_edge_attr)
            elif cfg.dataset.name == 'ogbg-molhiv':
                if cfg.posenc_edge.harmonic == 'multiple':
                    batch.extra_edge_attr = self.edge_encoder2(batch.extra_edge_attr)
                else:
                    batch.extra_edge_attr = self.edge_encoder(batch.extra_edge_attr)
                batch.edge_attr = batch.edge_attr + batch.extra_edge_attr
...
class GINEConvGraphGymLayer(nn.Module):
    ...
    def __init__(self, layer_config: LayerConfig, **kwargs):
        ...
        self.model = pyg_nn.GINEConv(gin_nn, edge_dim=edge_attr.size(-1))
