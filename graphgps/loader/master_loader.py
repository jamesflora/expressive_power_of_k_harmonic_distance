...
from graphgps.loader.dataset.citynetwork import citynetwork, EgoGraphDataset
...
def load_dataset_master(format, name, dataset_dir):
    ...
    if pe_enabled_list:
        ...
        if hasattr(dataset, 'edge_attr'):
            logging.info(f"Dataset edge attributes: {dataset.edge_attr.shape}")
        elif hasattr(dataset, 'extra_edge_attr'):
            logging.info(f"Dataset edge attributes: {dataset.extra_edge_attr.shape}")
        #global edge_size
        #edge_size = edge_attr.shape
...
def preformat_OGB_PCQM4Mv2(dataset_dir, name):
    ...
    dataset = PygPCQM4Mv2Dataset(root=dataset_dir, smiles2graph = smiles2graph)
...
def preformat_PCQM4Mv2Contact(dataset_dir, name):
    ...
    dataset = PygPCQM4Mv2Dataset(dataset_dir, subset='530k')
